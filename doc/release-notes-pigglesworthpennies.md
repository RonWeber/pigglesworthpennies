PigglesworthPennies Core version *0.15.1* is now available from:

  <https://download.pigglesworthpennies.org/pigglesworthpennies-0.15.1/>
...
installer (on Windows) or just copy over `/Applications/PigglesworthPennies-Qt` (on Mac)
or `pigglesworthpenniesd`/`pigglesworthpennies-qt` (on Linux).
...
A number of changes to the way PigglesworthPennies Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 PPP). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 PPP) in the next release.
