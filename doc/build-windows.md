...
Below are some notes on how to build PigglesworthPennies Core for Windows.

The options known to work for building PigglesworthPennies Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the PigglesworthPennies Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the PigglesworthPennies Core applications.
...
Note that for WSL the PigglesworthPennies Core source path MUST be somewhere in the default mount file system, for
example /usr/src/pigglesworthpennies, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/pigglesworthpennies-project/pigglesworthpennies.git
    sudo chmod -R a+rw pigglesworthpennies
...
way. This will install to `c:\workspace\pigglesworthpennies`, for example:

    make install DESTDIR=/mnt/c/workspace/pigglesworthpennies
...
causes two of the pigglesworthpennies executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the pigglesworthpennies code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the pigglesworthpennies source code).
